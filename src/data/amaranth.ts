...
// 단축근무(임산부) 전용 코드: D9/단=012(주간), E/단=013(야간), M/단=014(중간), N/단=015(심야)
const shiftToAmaranth: Record<string, string> = {
  ...
  // 시프트별 휴무 (신규 분리 코드)
  '#': '004',         // 구버전 기본 휴무 (D9 휴무로 처리)
  '#(주)': '004',
  '#(야)': '008',
  '#(중)': '009',
  '#(주6)': '010',
  '#(심야)': '011',
  // 모든 휴가는 D9(001) 근무일로 처리 (Amaranth에서 별도 휴가 신청)
  '#(연차)': '001',
  '#(대체)': '001', '#(병가)': '001', '#(공가)': '001',
  '#(보건)': '001', '#(경조)': '001', '#(생일)': '001', '#(출산)': '001',
  '#(육아)': '001', '#(태아)': '001', '#(창립기념일)': '001', '#(장기근속)': '001',
  '파견': '001', '출장': '001', '외근': '001',
  'D9/단': '012', 'E/단': '013', 'M/단': '014', 'N/단': '015',
};
